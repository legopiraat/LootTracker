...
class OgameClient[F[_] : ConcurrentEffect](executionContext: ExecutionContext, apiKeyConfig: ApiKeyConfig) extends OgameUrlBuilder {
  ...
  def getCombatReport(reportKey: ReportKey): F[Raid] = {
    val url = buildCombatUrl(reportKey, apiKeyConfig.apiKey)
  ...
  def getRecycleReport(reportKey: ReportKey): F[Raid] = {
    val url = buildRecycleUrl(reportKey, apiKeyConfig.apiKey)
  ...
  private[this] def combatReportToRaid(combatReport: CombatReport, reportKey: ReportKey): Raid = {
    val generic = combatReport.RESULT_DATA.generic
    val attacker = combatReport.RESULT_DATA.attackers.head.fleet_owner

    Raid(reportKey.reportKey, generic.loot_metal, generic.loot_crystal, generic.loot_deuterium, generic.event_time, attacker)
  }

  private[this] def recycleReportToRaid(recycleReport: RecycleReport, reportKey: ReportKey): Raid = {
    val generic = recycleReport.RESULT_DATA.generic
    Raid(reportKey.reportKey, generic.metal_retrieved, generic.crystal_retrieved, 0l, generic.event_time, generic.owner_name)
  }
